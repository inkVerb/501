...
if ( ($_SERVER['REQUEST_METHOD'] === 'POST') && (isset($_POST['piece'])) ) {
  ...
  if (isset($piece_id)) { // Editing piece
   ...
   if ($p_status == 'draft') { // No empty live date for publishing pieces
     ...
     $query = "UPDATE pieces SET type='$p_type_sqlesc', title='$p_title_sqlesc', slug='$p_slug_sqlesc', content='$p_content_sqlesc', after='$p_after_sqlesc', tags='$p_tags_sqljson', date_live=NULL, date_updated=NOW() WHERE id='$piece_id_sqlesc'";
     ...
     $p_live_sqlesc = escape_sql($p_live);
     $query = "UPDATE pieces SET type='$p_type_sqlesc', title='$p_title_sqlesc', slug='$p_slug_sqlesc', content='$p_content_sqlesc', after='$p_after_sqlesc', tags='$p_tags_sqljson', date_live='$p_live_sqlesc', date_updated=NOW() WHERE id='$piece_id_sqlesc'";
   }
