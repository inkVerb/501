...
if ( ($_SERVER['REQUEST_METHOD'] === 'POST') && (isset($_POST['piece'])) ) {
  ...
  if (isset($piece_id)) { // Editing piece
    ...
    if ($p_status == 'draft') { // No empty live date for publishing pieces
      ...
      $queryu = "UPDATE pieces SET type='$p_type_sqlesc', title='$p_title_sqlesc', slug='$p_slug_sqlesc', content='$p_content_sqlesc', after='$p_after_sqlesc', tags='$p_tags_sqljson', links='$p_links_sqljson', date_live=NULL, date_updated=NOW() WHERE id='$piece_id_sqlesc'";
      ...
      $p_live_sqlesc = escape_sql($p_live);
      $queryu = "UPDATE pieces SET type='$p_type_sqlesc', title='$p_title_sqlesc', slug='$p_slug_sqlesc', content='$p_content_sqlesc', after='$p_after_sqlesc', tags='$p_tags_sqljson', links='$p_links_sqljson', date_live='$p_live_sqlesc', date_updated=NOW() WHERE id='$piece_id_sqlesc'";
    }
    ...
    if ( ($p_status == 'publish') || ($p_status == 'update') ) {
      ...
      if (mysqli_num_rows($call) == 0) {
        ...
        if ( ($p_status == 'publish') && ($pubstatus == 'none') ) {
          $query = "INSERT INTO publications (piece_id, type, title, slug, content, after, tags, links, date_live, date_updated) VALUES ('$piece_id_sqlesc', '$p_type_sqlesc', '$p_title_sqlesc', '$p_slug_sqlesc', '$p_content_sqlesc', '$p_after_sqlesc', '$p_tags_sqljson', '$p_links_sqljson', '$p_live_sqlesc', NOW())";
          $callp = mysqli_query($database, $query);
          $query = "INSERT INTO publication_history (piece_id, type, title, slug, content, after, tags, links, date_live, date_updated) VALUES ('$piece_id_sqlesc', '$p_type_sqlesc', '$p_title_sqlesc', '$p_slug_sqlesc', '$p_content_sqlesc', '$p_after_sqlesc', '$p_tags_sqljson', '$p_links_sqljson', '$p_live_sqlesc', NOW())";
          ...
          $query = "UPDATE publications SET type='$p_type_sqlesc', pubstatus='published', title='$p_title_sqlesc', slug='$p_slug_sqlesc', content='$p_content_sqlesc', after='$p_after_sqlesc', tags='$p_tags_sqljson', links='$p_links_sqljson', date_live='$p_live_sqlesc', date_updated=NOW() WHERE piece_id='$piece_id_sqlesc'";
          $callp = mysqli_query($database, $query);
          $query = "INSERT INTO publication_history (piece_id, type, title, slug, content, after, tags, links, date_live, date_updated) VALUES ('$piece_id_sqlesc', '$p_type_sqlesc', '$p_title_sqlesc', '$p_slug_sqlesc', '$p_content_sqlesc', '$p_after_sqlesc', '$p_tags_sqljson', '$p_links_sqljson', '$p_live_sqlesc', NOW())";
          $callh = mysqli_query($database, $query);
          $query = "UPDATE pieces SET pub_yn=true WHERE id='$piece_id_sqlesc'";
...
// Check for GET or SESSION and validate in one if test
  ...
  // ID is not from SESSION, it is from GET
    ...
    $query = "SELECT type, status, title, slug, content, after, tags, links, date_live FROM pieces WHERE id='$piece_id_sqlesc'";
    $call = mysqli_query($database, $query);
    // Shoule be 1 row
    if (mysqli_num_rows($call) == 1) {
      // Assign the values
