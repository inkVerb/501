...
function checkPost($name, $value) {
  ...
  // Pieces
    ...
    $regex_replace = "/[^a-zA-Z0-9-]/";
    $result = preg_replace($regex_replace,"", $value);
    ...
    if (($value == 'live') || ($value == 'draft') || ($value == 'draft')) {
      $result = $value;
    } else {
      $result = 'draft';
    }
...
function formInput($name, $value, $errors) {
  ...
  // Piece forms
    ...
    $result = '<input type="text" id="p_title" name="p_title" value="'.$value.'"';
    ...
    <input type="text" id="p_status" name="p_status" value="'.$value.'"';
