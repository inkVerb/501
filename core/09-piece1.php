...
if ((isset($_GET['p'])) && (filter_var($_GET['p'], FILTER_VALIDATE_INT))) {
  ...
  $query = "SELECT title, slug, content, after, tags, links, date_live, date_updated FROM publications WHERE pubstatus='published' AND piece_id='$p_id'";
...
$row = mysqli_fetch_array($call, MYSQLI_NUM);
  ...
  if (!empty($links_array)) {
    ...
    // Display Links
    echo '<br><section class="links">'.$p_links.'</section><br>';

  }

  // Tags
  $tags_array = json_decode($p_tags_sqljson);
  // Only if we actually have tags
  if (!empty($tags_array)) {
    $p_tags = ''; // Start the $p_tags set
    // Parse $links_array into <a> tag variables
    foreach ($tags_array as $tag_item) {
      $tag_link = '<a href="tags.php?t='.$tag_item.'" title="View all #'.$tag_item.' posts" target="_blank" class="tag_item">#'.$tag_item.'</a>';
      $p_tags .= $tag_link.', ';
    }

    // Display Links
    echo '<br><section class="tags">'.$p_tags.'</section><br>';
