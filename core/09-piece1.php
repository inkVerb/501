...
if ((isset($_GET['p'])) && (filter_var($_GET['p'], FILTER_VALIDATE_INT))) {
  ...
  $query = "SELECT title, slug, content, after, date_live, date_updated FROM ${from_where}id='$p_id'";

} elseif ((isset($_GET['s'])) && (preg_match('/[a-zA-Z0-9-]{1,90}$/i', $_GET['s'])) && (!isset($_GET['preview']))) { // No preview for slug
