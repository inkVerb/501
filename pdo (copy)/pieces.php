...
$row = $pdo->select('pieces', 'status', 'live', 'id, type, status, pub_yn, title, date_live, date_created');
...
while ($row) {
  ...
  $draft_diff = ($pdo->rows == 0) ? '<code class="gray" title="Unpublished changes in draft, view in history or click \'Edit &rarr;\' > \'Update publication\' to publish"><i>(pending changes)</i></code>' : '';
