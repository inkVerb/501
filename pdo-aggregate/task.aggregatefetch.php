...
foreach ($rows as $row) {
  ...
  // Time to update?
  if (($next_update_php <= $curr_time_php) && ($f_status == 'active')) {
    // Fetch the feed
    ...
    foreach ($rss->channel->item as $item) {
      ...
      $f_live_sql = date("Y-m-d H:i:s", substr($epoch, 0, 10))
      ...
      $slug = $f_title;
      $regex_replace = "/[^a-zA-Z0-9-]/";
      $slug = strtolower(preg_replace($regex_replace,"-", $slug)); // Lowercase, all non-alnum to hyphen
      $slug = substr($slug, 0, 95); // Limit to 95 characters
      $f_slug_test_trim = DB::trimspace($slug);
      // Test database
      ...
      if ((isset($item->guid)) && ($item->guid != '')) {
        ...
        $query = $database->prepare("SELECT id FROM pieces WHERE slug=:slug");
        $query->bindParam(':slug', $f_guid_test_trim);
        $querypub = $database->prepare("SELECT id FROM publications WHERE slug=:slug");
        $querypub->bindParam(':slug', $f_guid_test_trim);
        $queryguid = $database->prepare("SELECT id FROM publications WHERE guid=:guid");
        $queryguid->bindParam(':guid', $f_guid_test_trim);
        $pdo->exec_($query);
        $piecesrows = $pdo->numrows;
        $pdo->exec_($querypub);
        $publicationsrows = $pdo->numrows;
        $pdo->exec_($queryguid);
        $pubguidrows = $pdo->numrows;

        if (($pubguidrows > 0) || ($f_slug_test_trim == 'feed')) {
          if (($piecesrows > 0) || ($publicationsrows > 0)) {
            $pub_update = true; // This seems to already exist, update this entry
          } else {
            $f_guid = $f_slug;
            // Check GUID again
            $queryguid = $database->prepare("SELECT id FROM publications WHERE guid=:guid");
            $queryguid->bindParam(':guid', $f_guid);
            $pdo->exec_($queryguid);
            $pubguidrows = $pdo->numrows;
            if ($pubguidrows > 0) {
      ...
      $f_live_trim = DB::trimspace($f_live);

      // Update or new entry?
      if ((isset($pub_update)) && ($pub_update = true)) {
        $query = $database->prepare("UPDATE publications SET type=:type, series=:series, title=:title, subtitle=:subtitle, slug=:slug, content=:content, excerpt=:excerpt, tags=:tags, feat_img=:feat_img, feat_aud=:feat_aud, feat_vid=:feat_vid, feat_doc=:feat_doc, date_live=:date_live WHERE guid=:guid");
      } else { // Insert into the database
        $query = $database->prepare("INSERT INTO publications
          (type, series, title, subtitle, slug, content, excerpt, guid, tags, feat_img, feat_aud, feat_vid, feat_doc, date_live) VALUES (:type, :series, :title, :subtitle, :slug, :content, :excerpt, :guid, :tags, :feat_img, :feat_aud, :feat_vid, :feat_doc, :date_live)");
      }
