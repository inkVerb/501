...
function feedSave(fID, blog_web_base) { // These arguments can be anything, same as used in this function
    ...
    AJAX.addEventListener( "load", function(event) { // This runs when AJAX responds
      ...
      if (jsonSeriesEditResponse["change"] == 'change') { // Change
        ...
        document.getElementById("view-series-"+fID).innerHTML = '<a id="view-series-'+fID+'" title="View the series this aggregates to" class="green" target="_blank" href="'+blog_web_base+'/series/'+jsonSeriesEditResponse["slug"]+'">view</a>';
        if (jsonSeriesEditResponse["status"] == 'active') {
          document.getElementById("refresh-div-"+fID).style.display = "inline";
        } else if (jsonSeriesEditResponse["status"] != 'active') {
          document.getElementById("refresh-div-"+fID).style.display = "none";
        }
  ...
  foreach ($rows as $row) {
    ...
    echo '<td id="sav-'.$agg_id.'">';
      ...
      if ($agg_status == 'problematic') {
        echo '<br><br><small class="red">RSS feed at this URL is invalid!</small>';
      } else {
            ...
            <tr>
              <td id="mcv-'.$agg_id.'">
                ...
                <div id="make-edits-'.$agg_id.'" style="display:none;">
                  ...
                  &nbsp;
                  <a id="view-series-'.$agg_id.'" title="View the series this aggregates to" class="green" target="_blank" href="'.$blog_web_base.'/series/'.$s_slug.'">view</a>
                  &nbsp;
                  <a id="view-list-'.$agg_id.'" title="List only aggregated publications" class="orange" target="_blank" href="'.$blog_web_base.'/blog.php?a='.$agg_id.'">list</a>
                  &nbsp;
                  <div id=refresh-div-'.$agg_id;
                  echo ($agg_status != 'active') ? ' style="display:none;"' : ' style="display:inline;"';
                  echo '><a id="refresh-'.$agg_id.'" title="Refresh this feed now" class="purple" href="'.$blog_web_base.'/act.processfetch.php?f='.$agg_id.'">refresh</a>
                </div>
              </td>
            </tr>
            <tr>
    ...
    echo '<small>Update interval: (in minutes)</small><br><input type="number" form="feed-edit-'.$agg_id.'" id="input-update-'.$agg_id.'" name="agg_update_interval" min="15" required value="'.$agg_update_interval.'">';
    echo '</td>';
    echo '</tr>';
