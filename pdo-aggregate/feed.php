...
if ((isset($_GET['s'])) && (preg_match($regex_match, $_GET['s'])) && ($_GET['s'] != 0)) { // Validate
  ...
  $feed_link = $blog_web_base.'/series/'.$series_slug;
...
<?xml-stylesheet type="text/xsl" href="$blog_web_base/rss.xsl" ?>
<rss version="2.0"
...
if ((isset($_GET['s'])) && (isset($series_id))) {
  $query = $database->prepare("SELECT piece_id, title, subtitle, slug, content, excerpt, series, aggregated, duration, tags, feat_img, feat_aud, feat_vid, feat_doc, feat_img_length, feat_aud_length, feat_vid_length, feat_doc_length, feat_img_mime, feat_aud_mime, feat_vid_mime, feat_doc_mime, date_live, date_updated FROM publications WHERE type='post' AND status='live' AND pubstatus='published' AND date_live<=NOW() AND series=:series ORDER BY date_live DESC LIMIT $blog_feed_items");
  ...
  $query = $database->prepare("SELECT piece_id, title, subtitle, slug, content, excerpt, series, aggregated, duration, tags, feat_img, feat_aud, feat_vid, feat_doc, feat_img_length, feat_aud_length, feat_vid_length, feat_doc_length, feat_img_mime, feat_aud_mime, feat_vid_mime, feat_doc_mime, date_live, date_updated FROM publications WHERE type='post' AND status='live' AND pubstatus='published' AND date_live<=NOW() ORDER BY date_live DESC LIMIT $blog_feed_items");
...
foreach ($rows as $row) {
  ...
  $p_aggregated_id = "$row->aggregated";
...
EOF;
    ...
    if ($feat_img_id != 0) {
      $feat_img_file_size = ($p_aggregated_id == 0) ? $feat_img_file_size : $p_feat_img_length;
      $feat_img_mime = ($p_aggregated_id == 0) ? $feat_img_mime : $p_feat_img_mime;
...
EOF;
    ...
    if ($feat_aud_id != 0) {
      $feat_aud_file_size = ($p_aggregated_id == 0) ? $feat_aud_file_size : $p_feat_aud_length;
      $feat_aud_mime = ($p_aggregated_id == 0) ? $feat_aud_mime : $p_feat_aud_mime;
...
EOF;
    ...
    if ($feat_vid_id != 0) {
      $feat_vid_file_size = ($p_aggregated_id == 0) ? $feat_vid_file_size : $p_feat_vid_length;
      $feat_vid_mime = ($p_aggregated_id == 0) ? $feat_vid_mime : $p_feat_vid_mime;
...
EOF;
      ...
      if ($feat_vid_mime == "video/mp4") {
        $feat_duration = (($p_aggregated_id != 0) && ($p_duration == 0)) ? $feat_vid_duration : $p_duration;
echo <<<EOF
\n  <itunes:duration>$feat_duration</itunes:duration>
EOF;
      }
    ...
    if ($feat_doc_id != 0) {
      $feat_doc_file_size = ($p_aggregated_id == 0) ? $feat_doc_file_size : $p_feat_doc_length;
      $feat_doc_mime = ($p_aggregated_id == 0) ? $feat_doc_mime : $p_feat_doc_mime;

echo <<<EOF
\n  <enclosure url="$feat_doc_url" length="$feat_doc_file_size" type="$feat_doc_mime" />
EOF;
