...
if ( ($_SERVER['REQUEST_METHOD'] === 'POST') && (!empty($_FILES)) && ($_FILES['upload_file']['size'][0] != 0) && (isset($_SESSION['user_id'])) ) {
    ...
    // File checks out
      ...
      if (move_uploaded_file($temp_file, $file_path_dest)) {
        ...
        switch ($upload_type) {
          case 'img':
            shell_exec('/srv/www/html/web/bash.imageprocess.sh '.$file_basename.' '.$file_extension.' '.$img_orientation.' '.$img_xs.' '.$img_sm.' '.$img_md.' '.$img_lg.' '.$img_xl);

          break;
          case 'svg':
            shell_exec('/srv/www/html/web/bash.imageprocess.sh '.$file_basename.' '.$file_extension.' svg');

          break;
          case 'vid':
            shell_exec('/srv/www/html/web/bash.videoprocess.sh '.$file_basename.' '.$file_extension);

          break;
          case 'aud':
            shell_exec('/srv/www/html/web/bash.audioprocess.sh '.$file_basename.' '.$file_extension);

          break;
          case 'doc':
            shell_exec('/srv/www/html/web/bash.documprocess.sh '.$file_basename.' '.$file_extension.' pdf');
