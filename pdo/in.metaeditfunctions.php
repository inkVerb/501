...
function piecesaction($action, $p_id) {
  ...
  switch ($action) {
    case 'unpublish':
      $pdo->try_update("UPDATE publications SET pubstatus='redrafting', date_updated=NOW() WHERE id='$p_id'");
      $call = $pdo->ok;
      if ($call) {
      ...
      break;
    case 'republish':
      $pdo->try_update("UPDATE publications SET pubstatus='published', date_updated=NOW() WHERE id='$p_id'");
      $call = $pdo->ok;
      if ($call) {
        $piecesactionsuccess = true;
      } else {
    ...
    case 'delete':
    case 'redelete':
      $pdo->try_update("UPDATE pieces SET status='dead', date_updated=NOW() WHERE id='$p_id'");
      $calld = $pdo->ok;
      $pdo->try_update("UPDATE publications SET status='dead', date_updated=NOW() WHERE piece_id='$p_id'");
      $callr = $pdo->ok;
      if (($calld) && ($callr)) {
        $piecesactionsuccess = true;
      } else {
    ...
    case 'restore':
    case 'undelete':
      $pdo->try_update("UPDATE pieces SET status='live', date_updated=NOW() WHERE id='$p_id'");
      $calld = $pdo->ok;
      $pdo->try_update("UPDATE publications SET status='live', date_updated=NOW() WHERE piece_id='$p_id'");
      $callr = $pdo->ok;
      if (($calld) && ($callr)) {
        $piecesactionsuccess = true;
      } else {
    ...
    case 'purge':
      ...
      if ($call2) {
        $pdo->try_delete("DELETE FROM publication_history WHERE piece_id='$p_id'");
        $call3 = $pdo->ok;
      }
      if ($call3) {
        $piecesactionsuccess = true;
